...
# ---------------------------------------------------------------------
async def main(vm_uri="ws://VM_IP:8765", mode="class", root_prompt="workout music"):
    ...
    try:
        async with websockets.connect(vm_uri, max_size=None) as ws:
            ...
            #     print(f"sent job {jid}  | prompt: {prompt}")
...
if __name__ == "__main__":
    ...
    from src.utils.prompt import choose_prompt
    style_choices = [
        "techno", "pop", "rock", "soul", "jazz", "classical",
    ]
    style_prompt = choose_prompt(style_choices, default_prompt="techno")
    ...
    # --- listening to the VM to play returned music -------------------------
    import argparse, asyncio
    p=argparse.ArgumentParser(); p.add_argument("--mode",default="class")
    VM_IP = "35.238.209.234"
    p.add_argument("--vm","--uri",dest="uri",default=f"ws://{VM_IP}:8765")
    args=p.parse_args()
    try:
        asyncio.run(main(args.uri,args.mode))
