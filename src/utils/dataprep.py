...
class ActivityDataset(Dataset):
    ...
    def __init__(self, data_path, annotation_col, statics, transform=None, sample_rate=64, sample_length_s=8, sliding_window_s=2, debug=False):
        ...
        # --- Store SubjectID for dataset splitting ---
        if 'SubjectID' not in df_data.columns:
            raise ValueError(
            "Data CSV must contain 'SubjectID' column for dataset splitting by subject."
            )
        # self.data will store a DataFrame with 'SubjectID' for each original sample.
        # This allows external splitting logic (like the one you provided)
        # to access subject IDs. The index of self.data aligns with the
        # original sample indices from df_data.
        self.data = df_data[['SubjectID']].copy()
